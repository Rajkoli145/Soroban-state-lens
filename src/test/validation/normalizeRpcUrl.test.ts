...
describe('normalizeRpcUrl', () => {
  ...
  it('should preserve pathname, query, and hash while removing trailing slash from path', () => {
    ...
    expect(normalizeRpcUrl('https://rpc.example.com/v1?query=1')).toBe('https://rpc.example.com/v1?query=1');
    expect(normalizeRpcUrl('https://rpc.example.com/v1/?query=1')).toBe('https://rpc.example.com/v1?query=1');
    expect(normalizeRpcUrl('https://rpc.example.com/v1#hash')).toBe('https://rpc.example.com/v1#hash');
    expect(normalizeRpcUrl('https://rpc.example.com/v1/?query=1#hash')).toBe('https://rpc.example.com/v1?query=1#hash');
  });
  ...
  it('should handle repeated trailing slashes in path correctly', () => {
    expect(normalizeRpcUrl('https://rpc.example.com/path//')).toBe('https://rpc.example.com/path');
  });
});
